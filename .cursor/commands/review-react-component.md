...
Please review this React component and update it to conform with the following conventions and design principles and libraries if it is lacking in any areas.

## Data fetching

- the component must use a hook to access any external data sources
  - this hook must use tanstack query (aka react query) to access the external data source
  - this hook must return the tanstack query or mutation object
- the component should make use of the hook's isLoading and isError properties to display considerate loading and error UI
- the loading UI should as must as reasonably possible resemble the UI once the data is loaded, using animation or opactiy to indicate that data is loading
- the error UI should explain in words the user will understand what has failed and give them an option to retry

## UI

- shadcn components should be used for all basic ui comonents, including form inputs and layout elements
- tailwind should be used for styling typography
- shadcn charts should be used for data vidualisation
- when applying tailwind classes, prefer to use the foreground and backgroud helper classes to maintain light and dark mode support
