...
This document describes how data flows through the LEGO marketplace scraper system, including all four pipeline jobs: **Capture**, **Materialize**, **Enrich**, and **Analyze**.

## Pipeline Overview

The system processes data through four sequential stages that transform raw marketplace API responses into analyzed, structured listing data.
...
- **`jobs`**: Job tracking for async operations
  - Job types: `ebay_refresh_listings`, `ebay_materialize_listings`, `ebay_enrich_listings`, `analyze_listings`
